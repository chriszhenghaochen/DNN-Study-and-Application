...
import matplotlib
import pandas as pd
from barclass1 import barchart
from lasso import LassoManager
from lasso import Datum
...
class SOM(object):
    ...
    def __init__(self,filename,comfilename,name, Data, labels = None, mapsize = None, norm_method = 'var',initmethod = 'pca',neigh='Guassian'):
        """
...
#           plt.scatter(X, msz[0]-1-Y, s=area, alpha=0.9,c='None',marker='o',cmap='jet',linewidths=3, edgecolor = 'r')
    ...
    def view_U_matrix(self,distance2=1,row_normalized='No',show_data='Yes',contooor='Yes',blob = 'No',save='No',save_dir = '', text_show = 'No', write = 'No', HCI = 'No', compare = 2, HCIcontoor ='No'):
        import scipy
...
#           umat[umat<=mn]=mn
        ...
        if show_data=='Yes':
            ...
            if text_show == 'Yes':
                for i in range(len(self.labels)):
                    plt.annotate(self.labels[i], xy = (coord[i,1], coord[i,0]), fontsize=10)


            #modified
            if write == 'Yes':
                f = open('C:/Users/zche2_000/Documents/NICTA project/data/bmu.txt','w')

                for i in range(len(self.labels)):
                    f.write(str(coord[i,1]) + "," + str(coord[i,0]) + "," + self.labels[i] + "\n") # python will convert \n to os.linesep
        ...
        if HCI == 'Yes':
            ...
            y = coord[:,0]
            l = self.labels
            lassodata = [Datum(*xyl) for xyl in zip(x,y,l)]
            names = []

            #modified 4-1 if not compare produce simple barchart
            if compare == 0:
                x = coord[:,1]
                y = coord[:,0]
                l = self.labels
                ...
                lman1 = LassoManager(ax, lassodata)
                plt.show()
                ...
                for i in range(len(data)):
                    if lman1.data[i].color == Datum.colorin:
                ...
                bc = simplebarchart(names1 = bararr1, filename= self.filename)
                # bc.process()
                bc.process(grouping = 'Yes')

                #communication
                com = Com(group1 = bararr1, group2 = bararr1,filename = self.comfilename )
                com.process()

                #write to a file

                f = open('E:/data/outputlab.csv','w')

                for i in range(len(bararr1)):
                    f.write(str(bararr1[i]) + "\n")

                f.close()

            #modified 4-2 if need compare produce complex barchart
            else:
                for i in range(compare):
                    ...
                    lman = LassoManager(ax, lassodata)
                    plt.show()

                    tmpname = []
                    for j in range(len(data)):
                        if lman.data[j].color == Datum.colorin:
                            # print lman1.data[i].labour
                ...
                #procssing bar
                bc = barchart(names1 = names[0], names2 = names[1],filename = self.filename)
                # bc.process()
                bc.process(grouping = 'Yes')

                #communication
                com = Com(group1 = names[0], group2 = names[1], filename= self.comfilename, grouping = 'Yes')
                com.process()

                com = Com(group1 = names[0], group2 = names[1], filename= self.comfilename)
                com.process()

                #write to a file
                f = open('E:/data/outputlab1.csv','w')

                for i in range(len(names[0])):
                    f.write(str(names[0][i]) + "\n")

                f.close()

                #write to a file
                f = open('E:/data/outputlab2.csv','w')

                for i in range(len(names[1])):
                    f.write(str(names[1][i]) + "\n")

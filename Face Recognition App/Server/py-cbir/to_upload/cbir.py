...
modelDir = osp.join('/home/jjs/wa/face/openface/models/dlib', "shape_predictor_68_face_landmarks.dat")
align = openface.AlignDlib(modelDir)
netDir = osp.join('/home/jjs/wa/face/openface/models/openface', 'nn4.small2.v1.t7')
net = openface.TorchNeuralNet(netDir, imgDim=96, cuda=True)
    ...
    def match_triN(self, img_dst):
        ...
        for bb in bbs:
            ...
            if phash in self.trinet_index:
                name = self.trinet_index[phash]['imgpath'].split('/')[-2].replace('_', ' ')
                
                identities.append((self.trinet_index[phash]['imgpath'], name, '0.0', urldat[name]))

                print('found in db!')
          
            for k, v in self.trinet_index.items():
                name = v['imgpath'].split('/')[-2].replace('_', ' ')
                identities.append((v['imgpath'], name, '{:.3f}'.format(embed_dist(np.array(v['vec']), rep)), urldat[name]))
